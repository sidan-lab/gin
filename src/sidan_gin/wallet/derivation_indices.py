class DerivationIndices:
    ...
    def payment(cls, account_index: int, key_index: int):
        ...
        return DerivationIndices.raw_path(["1852'", "1815'", f"{str(account_index)}'", str(0), str(key_index)])

    @classmethod
    def stake(cls, account_index: int, key_index: int):
        """Creates derivation indices for stake accounts"""
        return DerivationIndices.raw_path(["1852'", "1815'", f"{str(account_index)}'", str(2), str(key_index)])

    @classmethod
    def drep(cls, account_index: int, key_index: int):
        """Creates derivation indices for drep accounts"""
        return DerivationIndices.raw_path(["1852'", "1815'", f"{str(account_index)}'", str(3), str(key_index)])
